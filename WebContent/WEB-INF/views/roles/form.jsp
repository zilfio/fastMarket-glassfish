		...
		<c:choose>
			<c:when test="${not empty requestScope.create}">
				<h1 class="page-header">
					<spring:message code="role.heading.create" />
				</h1>
			</c:when>
			<c:when test="${not empty requestScope.update}">
				<h1 class="page-header">
					<spring:message code="role.heading.update" />
				</h1>
			</c:when>
			<c:when test="${not empty requestScope.delete}">
				<h1 class="page-header">
					<spring:message code="role.heading.delete" />
				</h1>
			</c:when>
		</c:choose>
	</div>
...
<form:form modelAttribute="role"
	...
	<div class="form-group">
		<label for="name"><spring:message code="role.name"/></label>
